...
By adding a few some extra bits allows not only for SELECT queries on the data but also UPDATE/INSERT/DELETE on your couchdb docs within Postgres.  
It is also possible to use your couch views as tables.  
...
Modify the environment variables to your database settings:
...
These will override the settings in bin/index.js:
...
You'll also need to set the db settings in bin/index.js:

    pgclient = new pg.Client("postgres://postgres@192.168.59.103/kiwiprints");

## Database setup

The db create scripts are in utils/createTables.sql

psql -h 192.168.59.103 -p 5432 -d kiwiprints -U kiwiprints --password -a -f utils/createTables.sql

The sql used for inserting new records are in sql/index.js.

# Running the importer

The importer runs until you stop it. If there is an update to a current record, it will delete the record and then re-insert.

    ./source/kiwiprints-postgres-import/bin/index.js 
    
## Run as daemon

    forever start ./source/kiwiprints-postgres-import/bin/index.js 
    forever list
    forever stop 1    
    
# Old import script - no longer necessary.

Run the sql/refresh.sql script to delete current records and import everything:

    psql -d kiwiprints -a -f sql/refresh.sql 
    psql -h 192.168.59.103 -p 5432 -d kiwiprints -U postgres --password -a -f sql/refresh.sql
    
Set cron to update db:
    crontab -e
    
Enter the following:

    0 0 * * * /home/kiwiprints/update_reports.sh
    0 6 * * * /home/kiwiprints/update_reports.sh
    0 12 * * * /home/kiwiprints/update_reports.sh
    0 18 * * * /home/kiwiprints/update_reports.sh
    
View reports in the companion app, [kiwiprints-report-app](https://github.com/chrisekelley/kiwiprints-report-app)
