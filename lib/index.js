                ...
                if (err) {
                    ...
                    if (result.rows.length > 0) {
                      ...
                      //  	        console.log(pgtable + ": " + key + " updated _rev: " + doc._rev);
	                                deferred.resolve({"state":"Updated", "_id": doc._id, "rev": doc._rev});
        	                    }
	                        });

                        }else{
                                //console.log(pgtable + ": NOOP " + doc._id + " pg_rev: " + pg_rev_num + " = doc_rev: " + doc_rev_num);
                                deferred.resolve({"state":"No change", "_id": doc._id, "rev":doc._rev});
			}
                    } else {
                        //new doc need to add
                            ...
                            pgclient.query(sql, function(err, result) {
                                if (err) {
                                  ...
                                  //  console.log(pgtable + ": " + doc._id + " added");
                                    deferred.resolve({"state":"New doc", "_id": doc._id});
                                }
                            });
                        } else {
        ...
        function process_change(change, done) {
            ...
            promise
                .then(function(res, body) {
                    ...
                    events.emit('change.success', change, res, body);
                    done();
                })
                .fail(function(err) {
